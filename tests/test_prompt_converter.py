...
async def test_base64_prompt_converter() -> None:
    ...
    output = await converter.convert(prompt="test", input_type="text")
...
async def test_str_join_converter_none_raises() -> None:
    ...
    with pytest.raises(TypeError):
        assert await converter.convert(prompt=None, input_type="text")
...
async def test_str_join_converter_invalid_type_raises() -> None:
    ...
    with pytest.raises(ValueError):
        assert await converter.convert(prompt="test", input_type="invalid")  # type: ignore # noqa
...
async def test_str_join_converter_unsupported_type_raises() -> None:
    converter = StringJoinConverter()
    with pytest.raises(ValueError):
        assert await converter.convert(prompt="test", input_type="image_path")
...
async def test_unicode_confusable_converter() -> None:
    ...
    output = await converter.convert(prompt="lorem ipsum dolor sit amet", input_type="text")
...
async def test_character_replacement_converter() -> None:
    ...
    output = await converter.convert(prompt="Hello World !", input_type="text")
...
async def test_leetcode_converter() -> None:
    ...
    output = await converter.convert(prompt="mood", input_type="text")
...
async def test_azure_speech_text_to_audio_convert(
    ...
    with patch("logging.getLogger") as _:
        ...
        await converter.convert(prompt=prompt)
...
async def test_send_prompt_to_audio_file_raises_value_error() -> None:
    ...
    prompt = "     "
    with pytest.raises(ValueError):
        assert await converter.convert(prompt=prompt, input_type="text")  # type: ignore
...
async def test_add_text_image_converter_invalid_input_image() -> None:
    ...
    with pytest.raises(FileNotFoundError):
        assert await converter.convert(prompt="mock_image.png", input_type="image_path")  # type: ignore
...
async def test_add_text_image_converter() -> None:
    ...
    converted_image = await converter.convert(prompt="test.png", input_type="image_path")
