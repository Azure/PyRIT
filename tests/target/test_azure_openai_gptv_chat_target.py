...
@pytest.mark.asyncio()
async def test_convert_image_to_data_url_file_not_found(azure_gptv_chat_engine: AzureOpenAIGPTVChatTarget):
    with pytest.raises(FileNotFoundError):
        await azure_gptv_chat_engine._convert_local_image_to_data_url("nonexistent.jpg")


@pytest.mark.asyncio()
async def test_convert_image_with_unsupported_extension(azure_gptv_chat_engine: AzureOpenAIGPTVChatTarget):
    ...
    with pytest.raises(ValueError) as exc_info:
        await azure_gptv_chat_engine._convert_local_image_to_data_url(tmp_file_name)
...
async def test_convert_image_to_data_url_success(
    ...
    result = await azure_gptv_chat_engine._convert_local_image_to_data_url(tmp_file_name)
...
@pytest.mark.asyncio()
async def test_build_chat_messages_with_consistent_roles(azure_gptv_chat_engine: AzureOpenAIGPTVChatTarget):
    ...
    with patch.object(
        ...
        messages = await azure_gptv_chat_engine._build_chat_messages(entries)
...
@pytest.mark.asyncio
async def test_build_chat_messages_with_unsupported_data_types(azure_gptv_chat_engine: AzureOpenAIGPTVChatTarget):
    ...
    with pytest.raises(ValueError) as excinfo:
        await azure_gptv_chat_engine._build_chat_messages([PromptRequestResponse(request_pieces=[entry])])
