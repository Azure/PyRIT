...
async def test_best_UCT(simple_prompts:PromptDataset):
    ...
    #fuzzer_orchestrator._prompt_nodes[0]._visited_num = 
    
    UCT_scores = [1.0, 2.0,3.0,4.0]
    ...
    for index, node in enumerate(prompt_nodes):
        ...
        assert int(UCT_score) == UCT_scores[index]
        

#testcase for select
@pytest.mark.asyncio
async def test_select(simple_prompts: PromptDataset):
    ...
    # when creating the prompt nodes, mctc path and last choice
    prompt_templates = [prompt_template1.template,prompt_template2.template,prompt_template3.template,prompt_template4.template]
