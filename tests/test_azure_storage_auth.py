...
from azure.storage.blob import BlobSasPermissions, UserDelegationKey, generate_container_sas
...
async def test_get_sas_token(mock_blob_sas, mock_container_client, mock_blob_service_client, mock_get_user_delegation_key):
...
async def test_get_sas_token_no_url():
    ...
    with pytest.raises(ValueError, match="Azure Storage Container URL is not provided. The correct format is 'https://storageaccountname.core.windows.net/containername'."):
...
async def test_get_sas_token_invalid_url_scheme():
    ...
    with pytest.raises(ValueError, match="Invalid Azure Storage Container URL. The correct format is 'https://storageaccountname.core.windows.net/containername'."):
