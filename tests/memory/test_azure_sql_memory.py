...
from tests.mocks import get_azure_sql_memory, get_duckdb_memory
...
def test_embedding_data_column_types(memory_interface: AzureSQLMemory):
    ...
    expected_column_types = {
        "id": UNIQUEIDENTIFIER,
        "embedding": NVARCHAR, #  Per SQLAlchemy's docs, the JSON type is rendered as NVARCHAR(max) for SQL Server DDL
                               #  https://docs.sqlalchemy.org/en/20/dialects/mssql.html#sqlalchemy.dialects.mssql.JSON
    ...
    assert column_types["embedding"] in [
        NullType,
        NVARCHAR,  # See comment above about NVARCHAR type
