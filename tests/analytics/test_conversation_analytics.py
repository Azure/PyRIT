...
from pyrit.memory.memory_models import ConversationData, EmbeddingData, EmbeddingMessageWithSimilarity
...
def test_get_similar_chat_messages_by_embedding(mock_memory_interface):
    ...
    mock_memory_interface.get_all_memory.side_effect = lambda model: mock_data if model == EmbeddingData else conversation_entries

    analytics = ConversationAnalytics(memory_interface=mock_memory_interface)
    similar_messages = analytics.get_similar_chat_messages_by_embedding(chat_message_embedding=target_embedding, threshold=0.99)
