...
from pyrit.setup import BuiltInInitPath


class TestBuiltInInitPath:
    """Tests for the BuiltInInitPath class and its path accessors."""

    def test_foundry_ansi_attack_path_exists(self) -> None:
        """Test that the foundry ansi_attack path is correctly defined."""
        path = BuiltInInitPath.foundry.ansi_attack
        assert isinstance(path, pathlib.Path)
        assert path.name == "ansi_attack.py"
        assert "foundry" in str(path)
        assert path.exists(), f"Expected foundry ansi_attack path to exist: {path}"

    def test_defaults_converter_initialization_path_exists(self) -> None:
        """Test that the defaults converter_initialization path is correctly defined."""
        path = BuiltInInitPath.defaults.converter_initialization
        assert isinstance(path, pathlib.Path)
        assert path.name == "converter_initialization.py"
        assert "defaults" in str(path)
        assert path.exists(), f"Expected defaults converter_initialization path to exist: {path}"

    def test_defaults_scorer_initialization_path_exists(self) -> None:
        """Test that the defaults scorer_initialization path is correctly defined."""
        path = BuiltInInitPath.defaults.scorer_initialization
        assert isinstance(path, pathlib.Path)
        assert path.name == "scorer_initialization.py"
        assert "defaults" in str(path)
        assert path.exists(), f"Expected defaults scorer_initialization path to exist: {path}"

    def test_defaults_target_initialization_path_exists(self) -> None:
        """Test that the defaults target_initialization path is correctly defined."""
        path = BuiltInInitPath.defaults.target_initialization
        assert isinstance(path, pathlib.Path)
        assert path.name == "target_initialization.py"
        assert "defaults" in str(path)
        assert path.exists(), f"Expected defaults target_initialization path to exist: {path}"
...
class TestFoundryPaths:
    """Tests specifically for the _FoundryPaths class."""
...
class TestDefaultsPaths:
    """Tests specifically for the _DefaultsPaths class."""
    ...
    def test_defaults_base_path_is_correct(self) -> None:
