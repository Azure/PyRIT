...
@pytest.fixture
def groq_chat_engine(patch_central_database) -> GroqChatTarget:
    return GroqChatTarget(
...
@pytest.fixture
def groq_mock_return() -> ChatCompletion:
...
@pytest.mark.asyncio
async def test_complete_chat_async_return(groq_mock_return: ChatCompletion, groq_chat_engine: GroqChatTarget):
    with patch("openai.resources.chat.Completions.create") as mock_create:
        mock_create.return_value = groq_mock_return
        ret = await groq_chat_engine._complete_chat_async(
...
@pytest.mark.asyncio()
async def test_convert_image_to_data_url_file_not_found(groq_chat_engine: GroqChatTarget):
    with pytest.raises(FileNotFoundError):
        await groq_chat_engine._convert_local_image_to_data_url("nonexistent.jpg")


@pytest.mark.asyncio()
async def test_convert_image_with_unsupported_extension(groq_chat_engine: GroqChatTarget):
    ...
    with pytest.raises(ValueError) as exc_info:
        await groq_chat_engine._convert_local_image_to_data_url(tmp_file_name)
...
async def test_convert_image_to_data_url_success(
    mock_get_memory_instance, mock_serializer_class, mock_guess_type, mock_exists, groq_chat_engine: GroqChatTarget
):
    ...
    result = await groq_chat_engine._convert_local_image_to_data_url(tmp_file_name)
...
@pytest.mark.asyncio()
async def test_build_chat_messages_with_consistent_roles(groq_chat_engine: GroqChatTarget):
    ...
    with patch.object(
        ...
        messages = await groq_chat_engine._build_chat_messages(entries)
...
@pytest.mark.asyncio
async def test_build_chat_messages_with_unsupported_data_types(groq_chat_engine: GroqChatTarget):
    ...
    with pytest.raises(ValueError) as excinfo:
        await groq_chat_engine._build_chat_messages([PromptRequestResponse(request_pieces=[entry])])
...
async def test_send_prompt_async_empty_response_adds_to_memory(
    groq_mock_return: ChatCompletion, groq_chat_engine: GroqChatTarget
):
    mock_memory = MagicMock()
    mock_memory.get_conversation.return_value = []
    mock_memory.add_request_response_to_memory = AsyncMock()

    groq_chat_engine._memory = mock_memory
    ...
    groq_mock_return.choices[0].message.content = ""
    with patch.object(
        groq_chat_engine,
        "_convert_local_image_to_data_url",
        return_value="data:image/jpeg;base64,encoded_string",
        ...
        with patch("openai.resources.chat.AsyncCompletions.create", new_callable=AsyncMock) as mock_create:
            ...
            with pytest.raises(EmptyResponseException) as e:
                await groq_chat_engine.send_prompt_async(prompt_request=prompt_req_resp)
                groq_chat_engine._memory.get_conversation.assert_called_once_with(conversation_id="12345679")
                groq_chat_engine._memory.add_request_response_to_memory.assert_called_once_with(request=prompt_req_resp)
...
async def test_send_prompt_async_rate_limit_exception_adds_to_memory(
    groq_chat_engine: GroqChatTarget,
):
    mock_memory = MagicMock()
    mock_memory.get_conversation.return_value = []
    mock_memory.add_request_response_to_memory = AsyncMock()
    ...
    setattr(groq_chat_engine, "_complete_chat_async", mock_complete_chat_async)
    ...
    with pytest.raises(RateLimitError) as rle:
        await groq_chat_engine.send_prompt_async(prompt_request=prompt_request)
        groq_chat_engine._memory.get_conversation.assert_called_once_with(conversation_id="123")
        groq_chat_engine._memory.add_request_response_to_memory.assert_called_once_with(request=prompt_request)

    assert str(rle.value) == "Rate Limit Reached"


@pytest.mark.asyncio
async def test_send_prompt_async_bad_request_error_adds_to_memory(groq_chat_engine: GroqChatTarget):
    mock_memory = MagicMock()
    mock_memory.get_conversation.return_value = []
    mock_memory.add_request_response_to_memory = AsyncMock()
...
@pytest.mark.asyncio
async def test_send_prompt_async(groq_mock_return: ChatCompletion, groq_chat_engine: GroqChatTarget):
    with NamedTemporaryFile(suffix=".jpg", delete=False) as tmp_file:
        tmp_file_name = tmp_file.name
    assert os.path.exists(tmp_file_name)
    prompt_req_resp = PromptRequestResponse(
    ...
    with patch.object(
        ...
        with patch("openai.resources.chat.AsyncCompletions.create", new_callable=AsyncMock) as mock_create:
            mock_create.return_value = groq_mock_return
            response: PromptRequestResponse = await groq_chat_engine.send_prompt_async(prompt_request=prompt_req_resp)
...
async def test_send_prompt_async_empty_response_retries(
    ...
    with patch.object(
        ...
        with patch("openai.resources.chat.AsyncCompletions.create", new_callable=AsyncMock) as mock_create:
            mock_create.return_value = groq_mock_return
            groq_chat_engine._memory = MagicMock(MemoryInterface)
...
@pytest.mark.asyncio
async def test_send_prompt_async_rate_limit_exception_retries(groq_chat_engine: GroqChatTarget):
...
@pytest.mark.asyncio
async def test_send_prompt_async_bad_request_error(groq_chat_engine: GroqChatTarget):
...
def test_parse_chat_completion_successful(groq_chat_engine: GroqChatTarget):
    mock_response = MagicMock()
    mock_response.choices = [MagicMock()]
    mock_response.choices[0].message = MagicMock()
    mock_response.choices[0].message.content = "Test response message"
    result = groq_chat_engine._parse_chat_completion(mock_response)
    assert result == "Test response message", "The response message was not parsed correctly"


def test_validate_request_unsupported_data_types(groq_chat_engine: GroqChatTarget):
    ...
    with pytest.raises(ValueError) as excinfo:
        groq_chat_engine._validate_request(prompt_request=prompt_request)
...
def test_is_json_response_supported(groq_chat_engine: GroqChatTarget):
    assert groq_chat_engine.is_json_response_supported() is True


def test_is_response_format_json_supported(groq_chat_engine: GroqChatTarget):
    ...
    result = groq_chat_engine.is_response_format_json(request_piece)

    assert result is True


def test_is_response_format_json_no_metadata(groq_chat_engine: GroqChatTarget):
    request_piece = PromptRequestPiece(
        role="user",
        original_value="original prompt text",
        converted_value="Hello, how are you?",
