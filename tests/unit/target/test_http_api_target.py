...
async def test_send_prompt_async_file_upload():
    ...
    response_text = str(response.request_pieces[0].converted_value) if response.request_pieces[0].converted_value else str(response)
...
async def test_send_prompt_async_no_file():
    ...
    request_piece = PromptRequestPiece(
        role="user",
        original_value="dummy",
        converted_value="non_existent_file.pdf"
    )
    prompt_request = PromptRequestResponse(request_pieces=[request_piece])
...
async def test_send_prompt_async_validation():
    ...
    target = HTTPXApiTarget(
        http_url="http://example.com/validate/",
        method="POST",
        timeout=180
    )
