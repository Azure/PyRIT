...
class TestFlipAttackExecution:
    ...
    async def test_perform_attack_formats_objective_correctly(self, flip_attack, basic_context):
        ...
        with patch.object(
            FlipAttack.__bases__[0], "_perform_attack_async", new_callable=AsyncMock  # PromptSendingAttack
        ) as mock_perform:
            ...
            result = await flip_attack._perform_attack_async(context=basic_context)
...
class TestAttackLifecycle:
    ...
    async def test_execute_async_validation_failure_prevents_execution(self, mock_objective_target, basic_context):
        ...
        # Should raise AttackValidationException since prepended conversation is not allowed
