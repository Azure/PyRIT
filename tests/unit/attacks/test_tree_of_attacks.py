...
class MockNodeFactory:
    ...
    def create_nodes_with_scores(scores: List[float]) -> List[TreeOfAttacksNode]:
        ...
        return [
            MockNodeFactory.create_node(
                NodeMockConfig(
                    node_id=f"node_{i}",
                    objective_score_value=score
                )
            )
...
class AttackBuilder:
    ...
    def with_auxiliary_scorers(self, count: int = 1):
        ...
        self.auxiliary_scorers = [
            cast(Scorer, self._create_mock_scorer(f"MockAuxScorer{i}")) 
    ...
    def add_nodes_to_tree(context: TAPAttackContext, nodes: List[TreeOfAttacksNode], parent: str = "root"):
        ...
        for i, node in enumerate(nodes):
            ...
            context.tree_visualization.create_node(
                f"{context.current_iteration}{score_str}",
                node.node_id,
                parent=parent
            )
...
class TestTreeOfAttacksInitialization:
    ...
    def test_init_with_custom_tree_parameters(self, attack_builder):
        ...
        attack = attack_builder.with_default_mocks().with_tree_params(
            tree_width=5,
            tree_depth=10,
            branching_factor=3,
            batch_size=20
...
class TestPruningLogic:
    ...
    def test_prune_nodes_to_maintain_width_removes_lowest_scoring_nodes(
        ...
        remaining_scores = [
            node.objective_score.get_value()
            for node in context.nodes
            if node.objective_score is not None
        ]
...
class TestExecutionPhase:
    ...
    @pytest.mark.asyncio
    async def test_perform_attack_single_iteration_success(
        ...
        success_node = node_factory.create_node(
            NodeMockConfig(
                ...
                objective_score_value=0.9,
                objective_target_conversation_id="success_conv"
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_perform_attack_early_termination_on_success(
        self, attack_builder, node_factory, helpers
    ):
        ...
        attack = attack_builder.with_default_mocks().with_tree_params(
            tree_depth=5,
            tree_width=1
        ).with_threshold(0.8).with_prompt_normalizer().build()
    ...
    @pytest.mark.asyncio
    async def test_perform_attack_batch_processing(
        self, attack_builder, node_factory, helpers
    ):
...
class TestHelperMethods:
    ...
    def test_format_node_result(self, basic_attack, node_factory):
        ...
        incomplete_node = node_factory.create_node(
            NodeMockConfig(completed=False)
        )
        ...
        completed_node = node_factory.create_node(
            NodeMockConfig(objective_score_value=0.7)
        )
...
class TestTreeOfAttacksNode:
    ...
    def test_node_initialization(self, node_components):
        ...
        assert node.prompt_sent is False
    ...
    def test_node_duplicate_creates_child(self, node_components):
        ...
        assert child_node.prompt_sent is False
    ...
    async def test_node_send_prompt_json_error_handling(self, node_components):
        ...
        wrapped_exception = Exception(f"Error sending prompt with conversation ID: id")
    ...
    async def test_node_off_topic_detection(self, node_components):
        ...
        with patch.object(node, '_generate_red_teaming_prompt_async', 
                        new_callable=AsyncMock, return_value=test_prompt) as red_teaming_mock:
    ...
    async def test_node_auxiliary_scoring(self, node_components):
        ...
        def mock_score_response(*args, **kwargs):
            return {
                "objective_scores": [obj_score],
                "auxiliary_scores": [aux_score1, aux_score2]
            }
        
        with patch('pyrit.score.Scorer.score_response_with_objective_async',
                new_callable=AsyncMock, side_effect=mock_score_response):
            await node.send_prompt_async(objective="Test objective")
...
class TestTreeOfAttacksErrorHandling:
    ...
    async def test_attack_handles_all_nodes_failing(self, attack_builder, helpers, node_factory):
        ...
        for i in range(2):
            ...
            # Ensure send_prompt_async is properly set as AsyncMock
            node.send_prompt_async = AsyncMock(return_value=None)
    ...
    async def test_attack_continues_after_node_errors(self, attack_builder, node_factory, helpers):
        ...
        # Failing node
        fail_node = node_factory.create_node(NodeMockConfig(
            node_id="fail_node",
            completed=True,
            off_topic=False,
        ...
        fail_node.duplicate = MagicMock(return_value=node_factory.create_node(
            NodeMockConfig(node_id="fail_dup", objective_score_value=0.3)
        ))
        ...
        for i in range(2):
            success_node = node_factory.create_node(
                NodeMockConfig(
                    node_id=f"success_node_{i}",
                    objective_score_value=0.5 + i * 0.1
                )
            )
...
class TestTreeOfAttacksVisualization:
    ...
    def test_tree_visualization_structure(self, basic_attack, node_factory, helpers):
        ...
        # Second level nodes
        node_0_child_0 = node_factory.create_node(
            NodeMockConfig(node_id="node_0_child_0", parent_id="node_0")
        )
        node_0_child_1 = node_factory.create_node(
            NodeMockConfig(node_id="node_0_child_1", parent_id="node_0")
        )
        node_1_child_0 = node_factory.create_node(
            NodeMockConfig(node_id="node_1_child_0", parent_id="node_1")
        )
