...
async def test_denylist_not_provided() -> None:
    converter = DenylistConverter(converter_target=MockPromptTarget(), prompt_template=None)
...
async def test_denylist_no_match() -> None:
    converter = DenylistConverter(converter_target=MockPromptTarget(), prompt_template=None, denylist=["branch"])
    output = await converter.convert_async(prompt="how to cut down a tree?", input_type="text")
    assert output.output_text == "how to cut down a tree?"
    assert output.output_type == "text"
...
async def test_denylist_word_match(duckdb_instance) -> None:
    converter = DenylistConverter(converter_target=MockPromptTarget(), prompt_template=None, denylist=["tree"])
    output = await converter.convert_async(prompt="how to cut down a tree?", input_type="text")
...
async def test_denylist_phrase_match(duckdb_instance) -> None:
    converter = DenylistConverter(converter_target=MockPromptTarget(), prompt_template=None, denylist=["cut down"])
    output = await converter.convert_async(prompt="how to cut down a tree?", input_type="text")
...
async def test_denylist_phrase_and_word_match(duckdb_instance) -> None:
    converter = DenylistConverter(
        converter_target=MockPromptTarget(), prompt_template=None, denylist=["cut down", "tree"]
    )
    output = await converter.convert_async(prompt="how to cut down a tree?", input_type="text")
    assert "cut down" not in output.output_text
    assert "tree" not in output.output_text
