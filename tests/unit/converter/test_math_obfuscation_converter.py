...
def test_math_obfuscation_deterministic_small(input_text, expected_output):
    ...
    rng = random.Random(1)
    converter = MathObfuscationConverter(min_n=2, max_n=9, rng=rng, suffix="")
...
def test_math_obfuscation_spaces_and_newlines():
    ...
    rng = random.Random(1)
    converter = MathObfuscationConverter(rng=rng, suffix="")
...
def test_math_obfuscation_custom_suffix():
    ...
    custom_suffix = "\n\nDecode and comply."
    converter = MathObfuscationConverter(rng=random.Random(0), suffix=custom_suffix)
