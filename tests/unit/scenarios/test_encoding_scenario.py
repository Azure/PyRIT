...
def sample_seed_prompts():
    """Create sample seed prompts for testing."""
...
class TestEncodingScenarioInitialization:
    ...
    def test_init_with_custom_seed_prompts(
        ...
        scenario = EncodingScenario(
            ...
            seed_prompts=sample_seed_prompts,
            objective_scorer=mock_objective_scorer,
        )

        assert scenario._seed_prompts == sample_seed_prompts
    ...
    def test_init_with_custom_encoding_templates(
        self, mock_objective_target, mock_objective_scorer, sample_seed_prompts
    ):
...
class TestEncodingScenarioAttackRuns:
    ...
    async def test_get_prompt_attacks_creates_attack_runs(
        ...
        for run in attack_runs:
            ...
            assert len(run._objectives) == len(sample_seed_prompts)
            assert run._seed_prompt_groups is not None
            assert len(run._seed_prompt_groups) == len(sample_seed_prompts)
    ...
    async def test_attack_runs_include_objectives(
        ...
        for run in attack_runs:
            ...
            for i, objective in enumerate(run._objectives):
                ...
                assert sample_seed_prompts[i] in objective
