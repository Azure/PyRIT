...
def test_export_conversation_by_attack_id_file_created(
    ...
    attack1_id = sample_conversations[0]._attack_identifier["id"]
...
def test_export_all_conversations_with_scores_correct_data(sqlite_instance: MemoryInterface):
    ...
    try:
        with patch.object(sqlite_instance, "get_message_pieces") as mock_get_pieces:
            # Create a mock score
            ...
            # Create a mock piece with scores attached (scores are now eagerly loaded)
            mock_piece = MagicMock()
            mock_piece.id = "piece_id_1234"
            mock_piece.original_prompt_id = "1234"
            mock_piece.converted_value = "sample piece"
            mock_piece.scores = [mock_score]  # Scores are now attached directly to pieces
