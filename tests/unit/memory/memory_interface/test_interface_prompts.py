...
def test_duplicate_conversation_pieces_not_score(duckdb_instance: MemoryInterface):
    ...
    assert len(duckdb_instance.get_prompt_scores(attack_id=attack1.get_identifier()["id"])) == 2
    assert len(duckdb_instance.get_prompt_scores(attack_id=attack2.get_identifier()["id"])) == 2
...
def test_duplicate_conversation_excluding_last_turn_same_attack(duckdb_instance: MemoryInterface):
    attack1 = PromptSendingAttack(objective_target=MagicMock())
    conversation_id_1 = "11111"
    pieces = [
        PromptRequestPiece(
...
def test_duplicate_memory_attack_id_collision(duckdb_instance: MemoryInterface):
    attack1 = PromptSendingAttack(objective_target=MagicMock())
...
def test_get_prompt_request_pieces_attack(duckdb_instance: MemoryInterface):
    ...
    attack1_entries = duckdb_instance.get_prompt_request_pieces(attack_id=attack1.get_identifier()["id"])
