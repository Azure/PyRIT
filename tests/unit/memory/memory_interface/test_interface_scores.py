...
def test_get_scores_by_attack_id_and_label(
    ...
    db_score = duckdb_instance.get_prompt_scores(attack_id=sample_conversations[0].attack_identifier["id"])
...
def test_add_score_duplicate_prompt(duckdb_instance: MemoryInterface):
    ...
    duckdb_instance.duplicate_conversation(new_attack_id=new_attack_id, conversation_id=conversation_id)
    dupe_piece = duckdb_instance.get_prompt_request_pieces(attack_id=new_attack_id)[0]
