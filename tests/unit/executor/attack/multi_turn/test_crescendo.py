...
class TestIntegrationScenarios:
    ...
    async def test_complete_successful_attack_scenario(
        ...
        with patch.object(
            attack._conversation_manager, "apply_prepended_conversation_to_objective_async", return_value=mock_conversation_state
        ):
...
class TestEdgeCases:
    ...
    async def test_concurrent_context_isolation(
        ...
        with patch.object(
            attack._conversation_manager, "apply_prepended_conversation_to_objective_async", side_effect=[mock_state1, mock_state2]
        ):
...
class TestCrescendoConversationTracking:
    ...
    async def test_setup_tracks_adversarial_chat_conversation_id(
        ...
        with patch.object(
            attack._conversation_manager, "apply_prepended_conversation_to_objective_async", new_callable=AsyncMock
        ) as mock_update:
