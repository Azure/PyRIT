...
from pyrit.models import AttackOutcome, AttackResult, ConversationReference, PromptRequestResponse
...
class TestAttackStrategyInitialization:
    ...
    def test_init_creates_default_event_handler(self):
        ...
        class TestStrategy(AttackStrategy):
            def _validate_context(self, *, context): pass
            async def _setup_async(self, *, context): pass
            async def _perform_async(self, *, context): 
                return AttackResult(
                    conversation_id="test-conversation-id",
                    objective="Test objective", 
            ...
            async def _teardown_async(self, *, context): pass
...
class TestAttackStrategyExecution:
    ...
    async def test_execute_async_validates_memory_labels_type(self, mock_attack_strategy):
        ...
        with pytest.raises(Exception):  # get_kwarg_param should raise for invalid type
            await mock_attack_strategy.execute_async(
                objective="Test objective",
                memory_labels="invalid"  # Should be dict
            )
...
class TestDefaultAttackStrategyEventHandler:
    ...
    @pytest.mark.asyncio
    async def test_on_post_execute_calculates_execution_time(self, event_handler, sample_attack_context, sample_attack_result, mock_logger):
    ...
    @pytest.mark.asyncio
    async def test_on_post_execute_logs_success(self, event_handler, sample_attack_context, sample_attack_result, mock_logger):
    ...
    @pytest.mark.asyncio
    async def test_on_post_execute_logs_failure(self, event_handler, sample_attack_context, sample_attack_result, mock_logger):
    ...
    @pytest.mark.asyncio
    async def test_on_post_execute_logs_undetermined(self, event_handler, sample_attack_context, sample_attack_result, mock_logger):
