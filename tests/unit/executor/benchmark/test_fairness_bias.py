...
def mock_scorer() -> MagicMock:
    ...
    scorer.score_text_async.return_value = [mock_score]
    return scorer
...
@pytest.fixture
def invalid_benchmark_context() -> FairnessBiasBenchmarkContext:
    """Invalid benchmark context for error testing."""
    return FairnessBiasBenchmarkContext(
...
class TestFairnessBiasBenchmark:
    ...
    @pytest.mark.asyncio
    async def test_validate_context_invalid_num_experiments(
        self, mock_prompt_target: MagicMock, mock_scorer: MagicMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_extract_name_single_capitalized_word(
        self, mock_prompt_target: MagicMock, mock_scorer: MagicMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_perform_async_handles_scorer_error(
        self,
        mock_prompt_target: MagicMock,
        sample_benchmark_context: FairnessBiasBenchmarkContext,
        sample_attack_result: AttackResult,
        ...
        mock_scorer.score_text_async.side_effect = Exception("Scoring failed")
        ...
        with patch("pyrit.executor.benchmark.fairness_bias.PromptSendingAttack") as mock_attack_class:
            ...
            with patch("pyrit.executor.benchmark.fairness_bias.CentralMemory") as mock_memory_class:
                ...
                assert len(sample_benchmark_context.experiment_results) == 1
                experiment_result = sample_benchmark_context.experiment_results[0]
                assert experiment_result["score_category"] == "error"
                assert "Scoring failed" in experiment_result["score_rationale"]

    @pytest.mark.asyncio
    async def test_get_experiment_summary_with_results(
        self, mock_prompt_target: MagicMock, mock_scorer: MagicMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_experiment_summary_no_results(
        self, mock_prompt_target: MagicMock, mock_scorer: MagicMock
    ) -> None:
...
class TestFairnessBiasBenchmarkExecuteAsync:
    ...
    @pytest.mark.asyncio
    async def test_execute_async_validates_required_parameters(
        self, mock_prompt_target: MagicMock, mock_scorer: MagicMock
    ) -> None:
        ...
        with pytest.raises(ValueError):
            await benchmark.execute_async(story_type="story")

        # Should raise error when story_type is missing
        with pytest.raises(ValueError):
            await benchmark.execute_async(subject="doctor")
