...
class DuckDBMemory(MemoryInterface, metaclass=Singleton):
    ...
    def _create_engine(self, *, has_echo: bool) -> None:
    ...
    def insert_entry(self, entry: Base) -> None: # type: ignore
        ...
        with self.get_session() as session:
            try:
                ...
                session.rollback()
                logger.exception("Error inserting entry into the table: {e}")
    ...
    def update_entries(self, *, entries: list[Base], update_fields: dict) -> None: # type: ignore
        """
    ...
    def get_memories_with_conversation_id(self, *, conversation_id: str) -> list[ConversationData]:
        ...
        try:
            ...
            logger.exception(f"Unexpected error: Failed to retrieve ConversationData with conversation_id {conversation_id}")
    ...
    def get_memories_with_normalizer_id(self, *, normalizer_id: str) -> list[ConversationData]:
        ...
        try:
            ...
            logger.exception(f"Unexpected error: Failed to retrieve ConversationData with normalizer_id {normalizer_id}")
    ...
    def update_entries_by_conversation_id(self, *, conversation_id: str, update_fields: dict) -> bool:
        ...
        entries_to_update = self.query_entries(ConversationData, conditions=ConversationData.conversation_id == conversation_id)
