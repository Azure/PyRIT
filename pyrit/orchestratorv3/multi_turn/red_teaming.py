...
from pyrit.orchestratorv3.components.conversation_manager import (
    ConversationManager,
    ConversationState,
)
...
class RedTeamingStrategy(AttackStrategy[MultiTurnAttackContext, MultiTurnAttackResult]):
    ...
    def __init__(
        ...
        *,
        ...
        self._conversation_manager = ConversationManager(orchestrator_identifier=self.get_identifier())
        self._score_evaluator = ScoreEvaluator(
            use_score_as_feedback=use_score_as_feedback,
            scorer=objective_scorer,
        )

    async def _setup(self, *, context: MultiTurnAttackContext) -> None:
        """
    ...
    async def _perform_attack(self, *, context: MultiTurnAttackContext) -> MultiTurnAttackResult:
        """
        ...
        logger.info(
            f"Backtracking strategy: {self._backtracking_strategy.name if self._backtracking_strategy else 'None'}"
        )
        ...
        while context.executed_turns < context.max_turns and not context.achieved_objective:
            ...
            if (
                not context.achieved_objective
                and self._backtracking_strategy
                and context.executed_turns < context.max_turns
            ):
    ...
    async def _teardown(self, *, context: MultiTurnAttackContext) -> None:
    ...
    async def _generate_next_prompt(self, context: MultiTurnAttackContext) -> str:
        ...
        if response is None:
            logger.warning("Received None response from prompt normalizer")
    ...
    async def _build_adversarial_prompt(
        ...
        response = self._conversation_manager.get_last_message(
            conversation_id=context.session.adversarial_chat_conversation_id, role="assistant"
        )
        if not response:
    ...
    async def _send_to_target(self, context: MultiTurnAttackContext, prompt: str) -> PromptRequestPiece:
        ...
        if response is None:
            ...
            raise ValueError("Received None response from prompt normalizer")
    ...
    async def _evaluate_objective_achievement(
        self, context: MultiTurnAttackContext, response: PromptRequestPiece, score: Optional[Score]
    ) -> bool:
        """
        ...
        """
        return self._score_evaluator.is_objective_achieved(score=score)
    ...
    async def _apply_backtracking(self, context: MultiTurnAttackContext) -> None:
        ...
        strategy_name = self._backtracking_strategy.name if self._backtracking_strategy else 'None'
        logger.debug(f"Applying backtracking strategy: {strategy_name}")
        pass
    ...
    def _set_adversarial_chat_seed_prompt(self, *, seed_prompt: Union[str, SeedPrompt]) -> None:
        ...
        Args:
            seed_prompt (str): The seed prompt text or a SeedPrompt object
        """
    ...
    def _set_adversarial_chat_system_prompt_template(self, *, system_prompt_template_path: Union[str, Path]) -> None:
        ...
        Args:
            system_prompt_template_path (Path): Path to the YAML file containing the system prompt template
        Raises:
