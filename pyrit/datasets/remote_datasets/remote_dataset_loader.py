...
class RemoteDatasetLoader:
    ...
    def harm_categories(self) -> list[str]:
        """
        Returns a list of harm categories present in the dataset.

        None implies all categories (or unknown)
        """
        pass
    ...
    def _write_cache(self, cache_file: Path, examples: List[Dict[str, str]], file_type: str):
        """
        Write data to cache.
    ...
    def fetch_examples(
        ...
        cache_file = data_home / self._get_cache_file_name(source, file_type)

        if cache and cache_file.exists():
            return self._read_cache(cache_file, file_type)

        if source_type == "public_url":
            examples = self._fetch_from_public_url(source, file_type)
        elif source_type == "file":
            examples = self._fetch_from_file(source, file_type)

        if cache:
            self._write_cache(cache_file, examples, file_type)
        else:
