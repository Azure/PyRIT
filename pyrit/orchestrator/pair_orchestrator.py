...
class PromptAutomaticIterativeRefinementOrchestrator(Orchestrator):
    ...
    def __init__(
        ...
        *,
        ...
        self._attacker_prompt_template = PromptTemplate.from_yaml_file(self._attacker_system_prompt_template_path)
        self._judge_system_prompt_template_path = DATASETS_PATH / "orchestrators" / "pair" / "judge_system_prompt.yaml"
        self._judge_prompt_template = PromptTemplate.from_yaml_file(self._judge_system_prompt_template_path)
    ...
    async def _judge_response(self, *, text: str) -> PromptRequestResponse:
        ...
        judge_system_prompt = self._judge_prompt_template.apply_custom_metaprompt_parameters(
            goal=self._attacker_objective,
        )
        self._judge_text_model.set_system_prompt(system_prompt=judge_system_prompt, conversation_id=conversation_id)
        ...
        judge_response = await self._judge_text_model.send_prompt_async(prompt_request=judge_requests)
    ...
    async def _get_attacker_response_and_store(
        ...
        attacker_response = await self._prompt_normalizer.send_prompt_async(
            normalizer_request=NormalizerRequest(
                request_pieces=[
                    NormalizerRequestPiece(request_converters=[], prompt_value=target_response, prompt_data_type="text")
                ]
            ),
    ...
    async def _get_target_response_and_store(self, *, text: str) -> PromptRequestResponse:
        ...
        target_response = await self._prompt_normalizer.send_prompt_async(
            normalizer_request=NormalizerRequest(
                request_pieces=[
                    NormalizerRequestPiece(request_converters=[], prompt_value=text, prompt_data_type="text")
                ]
            ),
            target=self._target_text_model,
            conversation_id=str(uuid.uuid4()),
