...
from pyrit.executor.attack import AttackConverterConfig, AttackScoringConfig, ImplicareAttack
...
@deprecated(
    cast(
        LiteralString,
        deprecation_message(
            old_item="ImplicareAttackOrchestrator",
            new_item=ImplicareAttack,
            removed_in="v0.12.0",
        ),
    ),
)
class ImplicareAttackOrchestrator(PromptSendingOrchestrator):
    """
    .. warning::
        `ImplicareAttackOrchestrator` is deprecated and will be removed in **v0.12.0**;
        use `pyrit.executor.attack.ImplicareAttack` instead.

    Essentially, it adds a system prompt to the beginning of the conversation to as a single attack.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            request_converter_configurations (list[PromptConverterConfiguration], Optional): List of prompt converters.
                These are stacked in order on top of the standard converter.
