...
class FuzzerOrchestrator(Orchestrator):
    ...
    async def execute_fuzzer(self) -> PromptRequestPiece:
        ...
        if (self._current_query + len(self._prompts)) >= self._query_limit:
            ...
            return FuzzerResult(success=False, template = self._new_templates, prompt_target_conversation_id=conversation_ids, description ="Maximum query limit reached.")
        
        elif self._current_jailbreak >= self._jailbreak_goal:
            logger.info(f"Maximum number of jailbreaks reached.")
            return FuzzerResult(success=False, nodes = self._new_templates, prompt_target_conversation_id=conversation_ids, description = 'Maximum number of jailbreaks reached.')
        
        
        # 1. Select a seed from the list of the templates using the MCTS
        print(self._current_query + len(self._prompts))
        current_seed = self._select(self._initial_prompts_nodes)
        ...
        try:
            ...
            return FuzzerResult(success=False, nodes = self._new_templates, prompt_target_conversation_id=conversation_ids, description = 'Tried to apply to and failed even after retries as it didn\'t preserve the prompt placeholder.')
        ...
        if self._num_jailbreak > 0:  # successful jailbreak
            ...
            self._new_templates.append(target_template)
            return FuzzerResult(success = True, nodes = self._new_templates, prompt_target_conversation_id=conversation_ids_jailbreak, description='Successful Jailbreak' )
    ...
    def _best_UCT_score(self):
        """Function to compute the Upper Confidence Bounds for Trees(UCT) score for each seed. The highest-scoring seed will be selected as the next seed.
