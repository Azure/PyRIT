...
class FuzzerOrchestrator(Orchestrator):
    ...
    async def execute_fuzzer(self) -> PromptRequestPiece:
        ...
        if self._current_query >= self._max_query:
            ...
            return FuzzerResult(success=False, nodes = self._new_nodes, prompt_target_conversation_id=conversation_id, description ="Maximum query limit reached.")
        ...
        if self._num_jailbreak > 0:  # successful jailbreak
            ...
            return FuzzerResult(success = True, nodes = self._new_nodes, prompt_target_conversation_id=conversation_ids_jailbreak )
    ...
    def _best_UCT_score(self):
        ...
        """
        return lambda pn: pn._rewards  / (pn._visited_num + 1) + self._frequency_weight * np.sqrt(
    ...
    def _update(self, prompt_nodes: PromptNode):
        ...
        for prompt_node in reversed(self._mcts_selected_path):
            ...
            prompt_node._rewards += reward * max(
    ...
    async def _apply_template_converter(self, template:str):
        ...
        target_seed_obj = await self._template_converter.convert_async(prompt=template)
