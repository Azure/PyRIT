...
from pyrit.score.scorer_config import ScorerConfig, SelfAskLikertScorerConfig
...
class SelfAskLikertScorer(Scorer):
    ...
    def __init__(self, chat_target: PromptChatTarget, likert_scale_path: Union[str, Path]) -> None:
        likert_scale_path = Path(likert_scale_path).resolve() if isinstance(likert_scale_path, str) else likert_scale_path
        self._config = SelfAskLikertScorerConfig(likert_scale_path=likert_scale_path)

        self._prompt_target = chat_target
        self.scorer_type = "float_scale"
        self._system_prompt, self._score_category = self._config.vars
