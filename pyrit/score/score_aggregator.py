...
def _lift_binary(
    ...
    def aggregator(scores: Iterable[Score]) -> ScoreAggregatorResult:
        ...
        details = "\n".join(
            f"   {sep} {s.score_category}: {s.score_rationale or ''}"
            for s in scores
        )
...
def XOR_(scores: Iterable[Score]) -> ScoreAggregatorResult:
    scores = list(scores)
    true_count = sum(1 for s in scores if s.get_value())
    result = true_count % 2 == 1
    headline = (
        "An odd number of constituent scorers returned True in an XOR composite scorer."
        if result else
        "An even number of constituent scorers returned True in an XOR composite scorer."
    )
    details = "\n".join(
        f"  - {s.score_category}: {s.score_rationale or ''}"
        for s in scores
    )
    return ScoreAggregatorResult(value=result, rationale=f"{headline}\n{details}")
...
def MAJORITY_(scores: Iterable[Score]) -> ScoreAggregatorResult:
    ...
    headline = (
        ...
        if result else
        "A strict majority of constituent scorers did not return True."
    )
    details = "\n".join(
        f"  - {s.score_category}: {s.score_rationale or ''}"
        for s in scores
