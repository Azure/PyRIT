...
class TreeOfAttacksNode:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            adversarial_chat: The chat model for generating adversarial prompts.
            adversarial_chat_seed_prompt: Initial prompt for adversarial chat.
            adversarial_chat_prompt_template: Template for subsequent prompts.
            adversarial_chat_system_seed_prompt: System prompt for adversarial chat.
            desired_response_prefix: Prefix that responses should start with.
            objective_scorer: Scorer to evaluate if objective is achieved.
            on_topic_scorer: Optional scorer to check if prompts are on-topic.
            request_converters: Converters to apply to outgoing requests.
            response_converters: Converters to apply to incoming responses.
            auxiliary_scorers: Additional scorers for extra metrics.
            attack_id: Identifier for the attack.
            memory_labels: Optional labels for memory storage.
            parent_id: ID of parent node (None for root nodes).
            prompt_normalizer: Optional prompt normalizer for sending prompts.
        """
        # Store configuration
    ...
    async def send_prompt_async(self, objective: str) -> None:
        ...
        Args:
            objective: The attack objective describing what the attacker wants to achieve.
        """
    ...
    async def _generate_adversarial_prompt_async(self, objective: str) -> str:
        ...
        Returns:
            The generated adversarial prompt text.
        """
    ...
    async def _is_prompt_off_topic_async(self, prompt: str) -> bool:
        """
        Check if the generated prompt is off-topic.
        
        Args:
            prompt: The prompt to check.
            
        Returns:
            True if the prompt is off-topic (and node should be pruned), False otherwise.
        """
    ...
    async def _send_prompt_to_target_async(self, prompt: str) -> PromptRequestResponse:
        """
        Send the adversarial prompt to the objective target.
    ...
    async def _score_response_async(self, *, response: PromptRequestResponse, objective: str) -> None:
        """
        Score the target's response using all configured scorers.
        
        This method scores the response with:
        1. The objective scorer (required)
        2. All auxiliary scorers (optional)
        
        Args:
            response: The response to score.
            objective: The attack objective for context.
        """
    ...
    def _handle_json_error(self, error: InvalidJsonException) -> None:
        ...
        Args:
            error: The JSON exception that occurred.
        """
    ...
    def duplicate(self) -> TreeOfAttacksNode:
        ...
        Returns:
            A new TreeOfAttacksNode instance ready for independent execution.
        """
    ...
    async def _generate_first_turn_prompt_async(self, objective: str) -> str:
        ...
        return self._adversarial_chat_seed_prompt.render_template_value(
            objective=objective
        )
    ...
    async def _generate_subsequent_turn_prompt_async(self, objective: str) -> str:
        ...
        Returns:
            The rendered template prompt text.
            
        Raises:
            RuntimeError: If no assistant response is found in the conversation.
        """
    ...
    async def _get_response_score_async(self, response_id: str) -> str:
        ...
        Args:
            response_id: The ID of the response to get the score for.
            
        Returns:
            The score value as a string, or "unavailable" if not found.
        """
    ...
    async def _send_to_adversarial_chat_async(self, prompt_text: str) -> str:
        ...
        Returns:
            The adversarial chat's response.
        """
    ...
    def _parse_red_teaming_response(self, red_teaming_response: str) -> str:
        ...
        Args:
            red_teaming_response: The raw response from the red teaming chat.
            
        Returns:
            The extracted prompt text.
            
        Raises:
            InvalidJsonException: If the response is not valid JSON or missing the prompt field.
        """
