...
class CrescendoAttack(AttackStrategy[MultiTurnAttackContext, AttackResult]):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            max_backtracks: Maximum number of times to backtrack during the attack. Must be positive.
            prompt_normalizer: The prompt normalizer to use for sending prompts. Defaults to None.

        Raises:
            ValueError: If max_backtracks is not positive or if objective target is not a PromptChatTarget.
        """
        ...
        if not isinstance(objective_target, PromptChatTarget):
            raise ValueError("Crescendo attack requires a PromptChatTarget")
        ...
        if not attack_scoring_config.objective_scorer:
            scoring_target = attack_adversarial_config.target  # Use adversarial chat as scoring target by default
    ...
    async def _setup_async(self, *, context: MultiTurnAttackContext) -> None:
        ...
        Args:
            context: Attack context with configuration
        """
    ...
    async def _perform_attack_async(self, *, context: MultiTurnAttackContext) -> AttackResult:
        ...
        logger.info(f"Starting crescendo attack with objective: {context.objective}")
        logger.info(f"Max turns: {context.max_turns}, Max backtracks:{self._max_backtracks}")

        # Attack Execution Steps:
        # 1) Generate progressively harmful prompts using adversarial chat
        # 2) Send the generated prompt to the target system
        # 3) Check if the target refused the prompt and backtrack if needed
        # 4) Evaluate the target's response using the objective scorer
        # 5) Check if the attack objective has been achieved
        ...
        while context.executed_turns < context.max_turns and not achieved_objective:
            ...
            # Score the response
            objective_score = await self._score_response_async(context=context, response=target_response)
            context.last_score = objective_score

            # Check if objective achieved
            achieved_objective = self._score_evaluator.is_objective_achieved(score=objective_score)
    ...
    async def _get_attack_prompt_async(
        ...
        *,
        ...
        Args:
            ...
            refused_text: Text that was refused by the target (if any).
        ...
        *,
        ...
        if refused_text:
            ...
            if last_message:
                ...
                prompt_parts.extend([
                    f"\nThe target responded to the last question with: {last_message.converted_value}",
                    "\n--------------------------------",
    ...
    async def _send_prompt_to_objective_target_async(
        ...
        *,
        ...
        Args:
            attack_prompt: The prompt to send.
            context: The attack context.
    ...
    async def _check_refusal_async(self, response: PromptRequestPiece, task: str) -> Score:
        ...
        Args:
            ...
            task: The task/prompt that was sent.
    ...
    async def _backtrack_memory_async(self, *, conversation_id: str) -> str:
        ...
        """
        # Access memory through the conversation manager's prompt normalizer
        memory = self._prompt_normalizer._memory
    ...
    def _retrieve_refusal_text_and_objective_score(
        ...
        for score in state.last_assistant_message_scores:
            ...
            if scorer_type == self._refusal_scorer.get_identifier()["__type__"]:
                logger.debug(f"Prepended response refusal score: {score.get_value()}")
                if score.get_value():
                    refused_text = state.last_user_message or ""
                    
            elif scorer_type == self._objective_scorer.get_identifier()["__type__"]:
                logger.debug(f"Prepended response objective score: {score.get_value()}")
    ...
    def _set_adversarial_chat_system_prompt_template(self, *, system_prompt_template_path: Union[Path, str]) -> None:
        ...
        Args:
            system_prompt_template_path: Path to the system prompt template.
    ...
    async def _generate_next_prompt_async(self, context: MultiTurnAttackContext) -> str:
        ...
        Args:
            context: The attack context containing the current state and configuration.
        ...
        if context.custom_prompt:
            logger.debug("Using custom prompt from prepended conversation")
            prompt = context.custom_prompt
            context.custom_prompt = None  # Clear for future turns
            return prompt

        # Generate prompt using adversarial chat
        logger.debug("Generating new attack prompt using adversarial chat")
    ...
    async def _perform_backtrack_if_refused_async(
        ...
        *,
        ...
        Args:
            context: The attack context.
            response: The target's response to check.
            prompt_sent: The prompt that was sent to the target.
        ...
        if context.backtrack_count >= self._max_backtracks:
            logger.debug(f"Backtrack limit reached ({self._max_backtracks}), continuing without backtracking")
            return False
        ...
        logger.info(f"Response refused, backtracking (attempt {context.backtrack_count + 1}/{self._max_backtracks})")
