...
class Prompt(abc.ABC):
    ...
    def __init__(
        self, prompt_target: PromptTarget, prompt_transformer: PromptConverter, prompt_text: str, conversation_id: str
    ) -> None:
        if not isinstance(prompt_target, PromptTarget):
            raise ValueError("prompt_target must be a PromptTarget")

        if not isinstance(prompt_transformer, PromptConverter):
    ...
    def send_prompt(self, normalizer_id: str):
        ...
        self.prompt_target.send_prompt(
            normalized_prompt=self.prompt_transformer.convert(self.prompt_text),
